...
def insertCaves(malware, target_section, cave_size=0):
    ...
    malware_name = "/home/user/Desktop/Code_caves/Malware_15_rdata/" + output_str
    pe.write(malware_name)
    time.sleep(1)
    status = "success"

    row = [output_str, cave_location, cave_size, status,flag]
    with open("Cave15_Non_empty_rdata_caves.csv",'a', newline='') as csvfile:
...
for root, dirs, files in os.walk(directory):
    for filename in files:
        ...
        valid_count += insertCaves(full_name, '.rdata')
        #time.sleep(1)
print(valid_count)
