...
def insertCaves(malware, target_section, cave_size=0):
    ...
    for section in pe.sections:
        if section.Name.decode().rstrip('\x00') == target_section:
            ...
            changingRawAddress = section.PointerToRawData
            target_found = True
        
        if target_found == True and section.PointerToRawData > changingRawAddress:
    ...
    pe.__data__[(int(changingRawAddress)+int(temp_RawSize)+ int(raw_addition)+1):(int(original_size)+ int(raw_addition)+ 1)] = pe.__data__[(int(changingRawAddress)+int(temp_RawSize)):(int(original_size))]


    for data in pe.__data__[(int(changingRawAddress)+int(temp_RawSize)):(int(changingRawAddress)+int(temp_RawSize)+ int(raw_addition))]:
      data = 0
