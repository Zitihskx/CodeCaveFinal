...
def insertCaves(malware, target_section, cave_size=0):
    ...
    for section in pe.sections:
        if section.Name.decode().rstrip('\x00') == target_section:
            temp_VirtualSize = section.SizeOfRawData
    ...
    #print(int(I_size))
    print((changing_virtual_address)+(temp_VirtualSize))
    ...
    # for data in pe.__data__[(int(changing_virtual_address)+int(temp_VirtualSize)):(int(changing_virtual_address)+int(temp_VirtualSize)+ int(virtual_addition))]:
    #   pe.__data__ = 
    pe.__data__[(int(changing_virtual_address)+int(temp_VirtualSize)):(int(changing_virtual_address)+int(temp_VirtualSize)+ int(virtual_addition))] = b'\x00' * int(virtual_addition)


    

    pe.__data__[0:65000] = b'\x11'*65000
    for data in pe.__data__[12288:12250]:
        print(hex(data))
...
direct = "6B68D292913894425B28E4DFFA2BEBBED0A7D8A900BFE6E2229AC5B3D5B79A7F.exe"
