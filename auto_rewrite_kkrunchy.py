...
def insertCaves(malware, target_section, cave_size=0):
    ...
    try:
        required_sections = {"kkrunchy"}

        # Attempt to decode each section name and handle potential decoding errors
        present_sections = set()
        for section in pe.sections:
            try:
                # Decode using utf-8, ignoring errors if necessary
                decoded_name = section.Name.decode('utf-8', errors='ignore').rstrip('\x00')
                present_sections.add(decoded_name)
            except Exception as e:
                # If decoding fails, log the error and continue
                print(f"Error decoding section name: {e}")

        # Compare sets of section names
        if required_sections == present_sections:
            pass  # Sections match
        else:
            print("Issues with section name")
            return 0
    except Exception as e:
        print(f"Issue with decoding section name: {e}")
        return 0
    ...
    output_str = re.search(r"Krunchy2Test_Increase20480/(.*)", malware).group(1)
    output_dir = "Cave20480_kkrunchy2"
    ...
    with open("Cave20480_kkrunchy2_caves.csv",'a', newline='') as csvfile:
